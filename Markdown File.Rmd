...
unique_values_per_month <- function(col_name, df = Crimes) {
  df %>% # Check if a column is unique to each monthly dataset
...
```
Leeds and Bradford receive by far the most crimes reported (53,562 and 39,722, respectively), with Kirklees (24,422), Wakefield (22,231) and Calderdale (13,354) also receiving significant reports. The remaining ten LSOA areas only received a combined 119 reports - however the remainings LSOAs are not part of West Yorkshire. While this does not necessarily mean they are errors, it makes them less relevant to an analysis of trends in West Yorkshire.
```{r}
Crimes <- Crimes %>% filter(LSOA.area %in% c("Leeds", "Bradford", "Kirklees", "Wakefield", "Calderdale") | is.na(LSOA.area))
```
#### Location
```{r}
Crimes %>%
...
Crimes <- Crimes %>%  mutate(Outcome.group = case_when(
 ...
 Crimes %>%
   ...
   scale_x_continuous(labels = comma, expand = expansion(mult = c(0, 0.4))) +
   labs(
     title = "Number of Observations per Last.outcome.category",
     y = "LSOA Area",
     x = "Frequency")
```
In addition to the missing data, the majority of observations did not lead to any recorded outcome, with the most common outcome being 'unable to prosecute suspect' (63,380) and 'no suspect identified' (43,562). In a further 1,539 cases, further action was not deemed to be in the public interest. Just 15,434 reports (9.7% of observations) had a further outcome; 1,317 had confirmed action taken by the police, 3,414 had alternative action by external parties, and 10,703 had an unknown court result. This highlights the significant portion of reported crimes which are unable to concluded with significant action.
...
ggMarginal(ggplot(Crimes, aes(y = Latitude, x = Longitude,)) +
          ...
          labs(y = "Latitude", x = "Longitude") + ggtitle("Latitude vs Longitude, with Marginal Boxplots"), 
            type = "boxplot")
```
*Longitude* ranges between -99.3 and 98.2, whilst *Latitude* ranges between -96.4 and 99.5. However, as a map of England only ranges between approximately 49.5 to 55.5 latitude and -5.5 to 1.5 longitude, these values cannot be possible. The coordinates of these impossible values can instead be imputed using the LSOA.name to approximate the true location:
```{r}
...
ggplot(England.map) + geom_sf(fill="#FAFFFA") + geom_sf(data=Crimes.sf, aes(col=LSOA.area), alpha=0.5) + 
  scale_colour_manual(values=unname(cols25()))
```
## Multi-Variate Analysis:
Month, LatLong, LSOA.name/area, Crime.type, Last.outcome.category/Outcome.group
#### Trends over time:
Heatmap of crime reports each month:
```{r}
hex_grid <- st_make_grid(Crimes.sf, cellsize = 0.015, square = FALSE) %>% st_sf() %>%
  st_join(Crimes.sf) %>%
  group_by(geometry, Month.imputed) %>% 
  summarise(Crime.count = n(), .groups = "drop") %>%
  filter(Crime.count > 1) 

ggplot() + # Plot and facet by Month
  geom_sf(data = England.map, fill = "darkslateblue", color = "grey70") +
  geom_sf(data = hex_grid, aes(fill = Crime.count), color = NA) +
  scale_fill_viridis_c(option = "plasma", trans = "sqrt", name = "Crime Count") +
  facet_wrap(~ Month.imputed) + labs(title="Heatmap of Crime Reports, by Month") +
  theme_void() + theme(plot.title = element_text(hjust=0.5))
```

```{r}
monthly_summary <- Crimes.imputed %>%
  group_by(Month.imputed) %>%
  summarise(
    total_crimes = n(),
    target_crimes = sum(!(Outcome.group %in% c("Alternative Action", "Prosecution/Formal Action", "Court-result unknown")), na.rm = TRUE),
    pct_target = 100 * target_crimes / total_crimes
  ) %>%
  ungroup()

cor(monthly_summary$pct_target, monthly_summary$total_crimes, use = "complete.obs")

ggplot(monthly_summary, aes(y = total_crimes, x = pct_target)) +
  geom_point(color = "darkblue", size = 3) + 
  geom_path(color = "blue", size = 1, linetype = "dotted") +  # connected line with arrows
  geom_text(aes(label = format(Month.imputed, "%Y-%m")), vjust = -1, size = 3) +  # label points with month-year
  labs(
    title = "Monthly Outcomes with No Recorded Action vs Number of Crimes",
    y= "Total Number of Crimes",
    x = "Percentage of No-Action Outcomes (%)"
  ) 
```

```{r}
Crimes.imputed %>% 
  group_by(Month.imputed, LSOA.area, LSOA.name) %>%
  summarise(Count = n(), .groups = 'drop') %>%
  ggplot(aes(x = Month.imputed, y = Count, col = LSOA.name)) +
  geom_line(linewidth = 1) +
  facet_wrap(~ LSOA.area, scales = "free_y") + 
  labs(title = "Crimes Reported by LSOA Name, Faceted by LSOA Area",
    y = "Crimes Reported", x = "Month") +  theme(legend.position = "none") 
```
```{r}
Crimes.imputed %>% #
  group_by(Month.imputed, Crime.type) %>%
  summarise(Count = n(), .groups = 'drop') %>%
  ggplot(aes(x=Month.imputed, y=Count, col=reorder(Crime.type, -Count))) +
  geom_line(linewidth=1.2) +
  scale_colour_manual(values=unname(cols25())) + 
  labs(title = "Crimes Reported by Crime Type, Over Time", y = "Crimes Reported", x = "Month", colour = "Crime Type")
...
```{r}
Crimes.imputed %>%
  group_by(Month.imputed, Outcome.group) %>%
  summarise(Count = n(), .groups = 'drop') %>%
  ggplot(aes(x=Month.imputed, y=Count, fill=reorder(Outcome.group, -Count))) +
    geom_area() +
    scale_fill_manual(values=unname(cols25())) + # Adjust cols25() or palette if Outcome.group has many levels
    labs(title = "Crimes Reported by Outcome Group, Over Time", y = "Crimes Reported", x = "Month", fill = "Outcome Group")
```



```{r}
mosaicplot(~Crime.type + LSOA.area, data = Crimes.imputed, shade=TRUE) #
mosaicplot(~Crime.type + Outcome.group, data = Crimes.imputed[!is.na(Crimes.imputed$Last.outcome.category),], shade=TRUE) #
mosaicplot(~Outcome.group + LSOA.area, data = Crimes.imputed, shade=TRUE) #
```


```{r}
CA_type_area = Crimes.imputed %>% select(Crime.type, LSOA.area) %>% table() %>% as.matrix() %>% CA() #
CA_type_outcome = Crimes.imputed %>% select(Crime.type, Outcome.group) %>% table() %>% as.matrix() %>% CA() #
CA_outcome_area = Crimes.imputed %>% select(Outcome.group, LSOA.area) %>% table() %>% as.matrix() %>% CA()
```
